...
const FruitList = () => {
  ...
  return (
    <div
      className={`flex ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} min-h-screen`}
    >
      <div className="flex-1 p-4">
        ...
        <div className="flex flex-col space-y-4">
          {fruits.map((fruit) => (
            <div
              key={fruit.id}
              className={`w-90 p-4 rounded-lg flex items-center shadow-md transition-transform duration-300 transform hover:scale-100 ${
                isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'
              } relative border-2 border-gray-700`}
            >
              <img
                src={fruit.img}
                alt={fruit.name}
                className="w-20 h-20 rounded-full border-4 border-red-600 mr-4 transition-transform duration-300 transform hover:rotate-12"
              />
              ...
              <div className="flex flex-row items-center ml-4 space-x-3">
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center bg-red-700 text-white text-xl font-bold rounded-full hover:bg-red-800 transition-colors duration-300 ${
                ...
                <button
                  onClick={() => handleQuantityChange(fruit.id, 1)}
                  className={`w-10 h-10 flex items-center justify-center bg-green-700 text-white text-xl font-bold rounded-full hover:bg-green-800 transition-colors duration-300 ${
                    isDarkMode ? 'border border-gray-800' : 'border border-gray-500'
                  }`}
                >
                  +
